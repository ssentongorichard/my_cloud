...
router.get("/UrbanFarmerList", async (req,res)=> {
    try{
        let farmerones = await Registration.find({ role: "UrbanFarmer"});
        res.render("UrbanFarmerList", {farmerones:farmerones});
    } catch (error) {
        res.status(400).send("Unable to find Urban Farmer in the database")
    }
})
